...
export default function StationSales() {
  ...
  function calculateChartDataAux(data, type, siteAux) {
    const sites = [...new Set(data.map((obj) => obj["SITE"]))]; // Obtener todos los sitios únicos
    let trlTime = data.map((obj) => obj[type]);
    let id = 1;
    let uniqueTrlTime = trlTime.filter(
      (time, index) => trlTime.indexOf(time) === index
    ...
    const totalFuelVolume = fuelVolumes.reduce((acc, vol) => acc + vol, 0);
    const chartDataBySite = sites.map((site) => {
      const siteData = data.filter((obj) => obj["SITE"] === site); // Filtrar los objetos que pertenecen al sitio actual

      let trlTime = siteData.map((obj) => obj[type]);
      let uniqueTrlTime = trlTime.filter(
        (time, index) => trlTime.indexOf(time) === index
      );
      const time = uniqueTrlTime.sort(compareHours);
      ...
      let fuelVolumes = time.map((trlTime) => {
        let filteredArr = siteData.filter((obj) => obj[type] === trlTime);
        let sum = filteredArr.reduce((acc, obj) => acc + obj.FUELVOLUME, 0);
        return sum;
      });
      let colorPicked;
    ...
    const filteredChartDataBySite = chartDataBySite.filter((siteData) => {
      if (siteAux === "TODAS") {
        return true;
      } else {
        return siteData.label === siteAux;
      }
    });
    console.log(filteredChartDataBySite);
    return [time, filteredChartDataBySite, totalFuelVolume];
  }
  useEffect(() => {
    if (loading === "success") {
