...
export default function TotalSales() {
  ...
  function calculateChartData(data, type) {
    ...
    const chartData = fuelVolumes;
    const totalFuelVolume = fuelVolumes.reduce((acc, vol) => acc + vol, 0);
    return [time, chartData, totalFuelVolume];
  }
  useEffect(() => {
    if (loading === "success") {
      const [daysAux, chartDataAux, totalFuelVolume] = calculateChartData(
        historyData,
        "TRLDAY"
      );
      setDays(daysAux);
      ...
      const [hoursAux, chartHourDataAux] = calculateChartData(
        historyData,
        "TRLHOUR"
      );
      const [weekday, chartWeekDayDataAux] = calculateChartData(
        historyData,
        "WEEKDAY"
      );
  ...
  useEffect(() => {
    if (ploading === "success") {
      const [daysPAux, chartDataPAux, totalFuelVolume] = calculateChartData(
        predictionData,
        "TRLDAY"
      );
      setDaysP(daysPAux);
      ...
      const [, chartPHourDataAux] = calculateChartData(
        predictionData,
        "TRLHOUR"
      );
      setChartDataPHour(chartPHourDataAux);
      const [, chartWeekDayDataAux] = calculateChartData(
        predictionData,
        "WEEKDAY"
      );
  ...
  const handleOptionChange = (event) => {
    ...
    if (event.target.value !== "TODAS") {
      fuelVolumes = calculateFuelVolumes(
        historyData,
        days,
        event.target.value,
        "TRLDAY"
      );
      fuelVolumesP = calculateFuelVolumes(
        predictionData,
        days,
        event.target.value,
        "TRLDAY"
      );
      fuelVolumesHour = calculateFuelVolumes(
        historyData,
        hour,
        event.target.value,
        "TRLHOUR"
      );
      fuelVolumesPHour = calculateFuelVolumes(
        predictionData,
        hour,
        event.target.value,
        "TRLHOUR"
      );
      fuelVolumesWeekday = calculateFuelVolumes(
        historyData,
        weekday,
        event.target.value,
        "WEEKDAY"
      );
      fuelVolumesPWeekday = calculateFuelVolumes(
        predictionData,
        weekday,
        event.target.value,
        "WEEKDAY"
      ...
      fuelVolumes = calculateFuelVolumes(historyData, days, "TODAS", "TRLDAY");
      fuelVolumesP = calculateFuelVolumes(
        predictionData,
        days,
        "TODAS",
        ...
        {value === 0 && (
          <div>
            {loading === "success" && ploading === "success" && (
              <LineChart
                days={days}
                dataSetsInfo={dataset(chartData, chartDataP)}
